define(['app'], function (app) {
    app.controller('dataManagerResearchCtrl', ['$scope', '$state', '$stateParams', '$timeout', 'ReqServ', 'ngDialog', 'appInterface', function ($scope, $state, $stateParams, $timeout, ReqServ, ngDialog, appInterface) {
        $scope.research = {activityType: '5'};
        $scope.researchInfo = {};
        $scope.uploadDataModel.value = '单个添加调研信息';
        $scope.uploadDataModel.template = 'editResearchTemplate';
        $scope.uploadDataModel.pageTitle = '批量添加调研信息';
        $scope.uploadDataModel.templateFile = appInterface.fileUrl + 'activity_dy.xlsx';
        ...
        var dataModel = {
            toolsBar: [{
                btnName: '添加调研信息', bgStyle: 'bg-purple-light text-purple', event: function () {
                    if ($scope.hasTaskRun($scope.researchInfo.loading))
                        return;
                    ReqServ.clearObj($scope.researchInfo);
                    $scope.researchInfo.activityType = '5';
                    $scope.researchInfo.pageTitle = '添加调研信息';
                    $scope.researchInfo.btnName = '添加';
                    ngDialog.open({
                        template: 'editResearchTemplate',
            ...
            controls: [{
                ...
                name: '编辑', event: function (item) {
                    if ($scope.hasTaskRun($scope.researchInfo.loading))
                        return;
                    _item = item;
                    ReqServ.copyObj(_item.source, $scope.researchInfo);
                    $scope.researchInfo.pageTitle = '编辑调研信息';
                    $scope.researchInfo.btnName = '保存';
                    ngDialog.open({
                        template: 'editResearchTemplate',
                        className: 'ngdialog-theme-default',
                        onOpenCallback: $scope.editOpenedCallback,
            ...
            heads: [{title: '日期', dataKey: 'researchDate', width: 90},
        ...
        var loadLocalData = function () {
            ...
            ReqServ.copyObj($scope.searchModel, $scope.research);
        ...
        var loadData = function () {
            $scope.loading.isShow = true;
            $scope.research.pageSize = $scope.searchModel.pageSize;
            $scope.research.beginDate = ReqServ.getDateNum($scope.research.startDateTime);
            $scope.research.endDate = ReqServ.getDateNum($scope.research.endDateTime);
            ReqServ.request('POST', 'ty/tyActivity/read/list', $scope.research).success(function (result) {
                ...
                for (var i = 0; i < result.data.list.length; i++) {
                    result.data.list[i].researchDate = ReqServ.getDateString(result.data.list[i].beginDate + '');
        ...
        $scope.searchResearch = function () {
            ReqServ.removeStore('data');
            $scope.research.pageNum = 1;
            $scope.clearData();
            ReqServ.copyObj($scope.research, $scope.searchModel);
            loadData();
        };
        $scope.editResearch = function () {
            ReqServ.setBtnStatus(true, '处理中...', null, $scope.researchInfo);
            $scope.researchInfo.beginDate = ReqServ.getDateNum($scope.researchInfo.researchDate);
            $scope.researchInfo.userIds = $scope.researchInfo.researcherId.split(',');
            ReqServ.request('POST', $scope.researchInfo.id ? 'ty/tyActivity/update' : 'ty/tyActivity/add', $scope.researchInfo).success(function (result) {
                ReqServ.setBtnStatus(false, $scope.researchInfo.id ? '保存' : '添加', null, $scope.researchInfo);
                if (result.httpCode != appInterface.successCode) {
                    ReqServ.setBtnStatus(false, $scope.researchInfo.id ? '保存' : '添加', null, $scope.researchInfo);
                    $scope.showAlert(result.msg ? result.msg : appInterface.alert);
                    return;
                }
                if ($scope.researchInfo.id) {
                    var teams = [], list = $scope.researchInfo.researcherId.split(',');
                    ...
                    $scope.researchInfo.teams = teams.join(',');
                    ReqServ.copyObj($scope.researchInfo, _item.source);
                    _item.item = [{text: _item.source.researchDate},
        ...
        $scope.editOpenedCallback = function () {
            $scope.researchInfo.dirty = false;
        };
        ...
        $scope.addCustomers = function () {
            $state.go('master.datamanager.customer', {id: _item.source.id, index: _item.index, model: 'research'});
        };
