...
//		}
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken)
		...
		List<SysUserBean> pageInfo = sysUserService.queryPbUser(params);
		if (pageInfo.size() == 1) {
			SysUserBean user = pageInfo.get(0);
			StringBuilder sb = new StringBuilder(100);
			for (int i = 0; i < token.getPassword().length; i++) {
				sb.append(token.getPassword()[i]);
			}
			if("1".equals(user.getStatus())){
				logger.warn("PB USER [{}] IS Frozen: {}", token.getUsername(), sb.toString());
				return null;
			}
			...
			logger.warn("PB USER [{}] PASSWORD IS WRONG: {}", token.getUsername(), sb.toString());
			return null;
		} else {
			logger.warn("No PB user: {}", token.getUsername());
			return null;
		}
	}
