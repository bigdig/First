...
public abstract class BaseServiceImpl<T extends BaseModel> implements BaseService<T>{
	public String CACHE_NAMESPACE = "ty:";
    ...
    public StringBuilder getCachePrefix(){
        ...
        String className = this.getClass().getSimpleName().replace("ServiceImpl", "");
        sb.append(className.substring(0, 1).toLowerCase()).append(className.substring(1));
        return sb;
    }
    
    public String getCacheId(T record){
    	return getCachePrefix().append(":").append(record.getId()).toString();
    }
    
	public void clearRecordCache(T record){
		redisService.remove(getCacheId(record));
	}
