...
public class WxTemplateMsgUtil {
	private Logger log = LoggerFactory.getLogger(WxTemplateMsgUtil.class);
	 ...
	 * @return
	 */
	public String deleteWXTemplateMsgById(String templateId) {
	 ...
	 * @return
	 */
	public String sendWechatMsgToUser(String touser, String templatId, String clickurl, JSONObject miniprogram,String topcolor,
			JSONObject data) {
		...
		try {
			JSONObject result = WX_HttpsUtil.httpsRequest(tmpurl, "POST", json.toString());
			JSONObject resultJson = new JSONObject(result);
			XxlJobLogger.log("发送微信消息返回结果：" + resultJson.toJSONString());
			String errmsg = (String) resultJson.get("errmsg");
