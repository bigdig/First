...
public class SynchronizeServiceOrgJobHandler extends IJobHandler {
			...
			try {
				...
				if(returnMap.get("andOrUpdate")!=-1){					
					...
					if(returnMap.get("andOrUpdate")!=0){
						dealFlag="2";
						remark = "同步未成功，机构返回值为："+returnMap.get("andOrUpdate");
					}
				}
				if(returnMap.get("delete")!=-1){					
					...
					if(returnMap.get("delete")!=0){
						dealFlag="2";
						remark = "同步未成功，机构返回值为："+returnMap.get("delete");
					}
				}
			} catch (Exception e) {
				succ = false;
	...
	private Map<String,Integer> addServiceorgToKM(TyServiceorg tyServiceorg,Map<String,Integer> returnMap) throws Exception {
		...
		XxlJobLogger.log("推送机构内容[" + JSONObject.toJSONString(customer) + "]");

		int returnResult=-1,returnResultDel=-1;
		CrmSynchroImplServiceTestCase crm = new CrmSynchroImplServiceTestCase();
		if(StringUtils.equals(tyServiceorg.getDeleteFlag(), "0")){			
		...
		if(StringUtils.equals(tyServiceorg.getDeleteFlag(), "1")){			
	...
	private Customer setObjectOfCustomer(TyServiceorg tyServiceorg) {
		...
		String serviceSaler = "";
		if(StringUtils.isNotBlank(tyServiceorg.getCreateBy())){
			sysUserCreate = sysUserService.selectByPrimaryKey(tyServiceorg.getCreateBy());
		}
		if(StringUtils.isNotBlank(tyServiceorg.getUpdateBy())){
			sysUserUpdate = sysUserService.selectByPrimaryKey(tyServiceorg.getUpdateBy());
		}
		if(StringUtils.isNotBlank(tyServiceorg.getServiceSaler())
				&&StringUtils.isNotBlank(tyServiceorg.getSalerId())){
			String[] salerIds = tyServiceorg.getSalerId().split(",");
			int count = 0;
		...
		customer.setManger(serviceSaler!=""?serviceSaler:tyServiceorg.getServiceSaler());
		...
		if(StringUtils.isNotEmpty(tyServiceorg.getOrgSimpleName())){
			customer.setCustSht(tyServiceorg.getOrgSimpleName());
		}else{
			customer.setCustSht(tyServiceorg.getOrgName());
		}
		...
		customer.setCreator(sysUserCreate!=null?sysUserCreate.getAccount():tyServiceorg.getCreateBy());
		customer.setUpdator(sysUserUpdate!=null?sysUserUpdate.getAccount():tyServiceorg.getUpdateBy());
	...
	private String transCustCat(String custCat) {
		...
		} else if ("8".equals(custCat))  {
			return Constants.CUST_CAT_8;
		} else if ("9".equals(custCat))  {
			return Constants.CUST_CAT_9;
		} else if ("10".equals(custCat))  {
			return Constants.CUST_CAT_10;
		} else if ("11".equals(custCat))  {
			return Constants.CUST_CAT_11;
		} else if ("12".equals(custCat))  {
			return Constants.CUST_CAT_12;
		} else if ("13".equals(custCat))  {
			return Constants.CUST_CAT_13;
		} else if ("14".equals(custCat))  {
			return Constants.CUST_CAT_14;
		} else if ("15".equals(custCat))  {
			return Constants.CUST_CAT_15;
		} else if ("16".equals(custCat))  {
			return Constants.CUST_CAT_16;
		} else {
			return custCat;
		}
	}
