 ...
 */
@JobHandler(value = "updateHistoryActivity")
@Component
public class UpdateImportActivityJobHandler extends IJobHandler {
    private Logger logger = LoggerFactory.getLogger(UpdateImportActivityJobHandler.class);
	...
	public ReturnT<String> execute(String param) throws Exception {
		...
		String yesterday = DateUtil.format(DateUtil.addDate(new Date(), Calendar.DAY_OF_MONTH, -1), DateUtil.DATE_PATTERN.YYYYMMDD);
		if(StringUtils.isNotBlank(param)){
			yesterday = param;
		}
		params.put("dataSrc", "1");
		params.put("yesterdayCreate", yesterday);
		
		List<TyActivity> results = tyActivityService.findByCondition(params);
		XxlJobLogger.log("处理导入的活动["+yesterday+"]! ，条数：" + results.size());
		...
		for(TyActivity act:results){
			//处理过的就中跳过
			if(StringUtils.contains(act.getDataSrcname(), Constants.DATA_SRC_HANDLED)){
				continue;
			}
			//1.更新活动为已经处理
			act.setDataSrcname(act.getDataSrcname() + Constants.DATA_SRC_HANDLED);
