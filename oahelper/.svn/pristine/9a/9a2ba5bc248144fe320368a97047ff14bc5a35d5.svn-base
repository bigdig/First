...
public class ConnectionPool implements Config {
	...
	public TrackerServer checkout() throws Exception {
		...
		if (trackerServer == null) {
			...
			if (trackerServer == null) {
				...
				throw new Exception("获取空闲连接(checkout)-error");
			}
		}
	...
	private void initClientGlobal() throws Exception {
		...
		if (StringUtils.isNotEmpty(PropertiesUtil.getString("fdfs_client_conf"))) {
			fdfsClientConfigFilePath = PropertiesUtil.getString("fdfs_client_conf");
		}
