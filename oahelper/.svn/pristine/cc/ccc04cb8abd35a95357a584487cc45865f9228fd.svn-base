...
public final class RedisUtil {
    ...
    public static final Serializable get(final String key,boolean expired) {
    	if(expired){    		
    		expire(key, EXPIRE);
    	}
    	return getRedis().opsForValue().get(key);
    ...
    public static final void set(final String key, final Serializable value,boolean expired) {
    	getRedis().boundValueOps(key).set(value);
    	if (expired){    		
    		expire(key, EXPIRE);
    	}
    ...
    public static final Boolean existsNoExpire(final String key) {
    	return getRedis().hasKey(key);
    }
