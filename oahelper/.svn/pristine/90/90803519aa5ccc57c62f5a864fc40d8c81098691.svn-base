...
util.showThisRoute = function (itAccess, currentAccess) {
  if (typeof itAccess === 'object' && Array.isArray(itAccess)) {
    if (typeof currentAccess === 'object' && Array.isArray(currentAccess)) {
      let intersect = util.intersect(itAccess, currentAccess);
...
util.intersect = function (arr1, arr2) {
...
util.setCurrentPath = function (vm, name) {
  ...
  if (name === 'home_index') {
    ...
    let currentPathObj = util.getRouterObjByName(vm.$store.state.app.routers, name)
